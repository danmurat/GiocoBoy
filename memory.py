class Memory:
    def __init__(self):
        ...
        self.ie = [0] * 0x1  # interrupt enable register

    def load_rom(self, rom_data):
        self.rom_data = rom_data
        # print(f"ROM Size: {len(rom_data)} bytes")
        # print(" ".join([hex(x)[2:].zfill(2) for x in rom_data[0x4000:0x4016]]))
    ...
    def read_byte(self, addr):
        ...
        if 0x0000 <= addr <= 0x3FFF:  # ROM Bank 0 (fixed)
            # print(
            #     f"ROM Bank 0: Address: {addr:04X}, Data: {self.rom_data[addr]:02X}")
            return self.rom_data[addr]
        elif 0x4000 <= addr <= 0x7FFF:  # ROM Bank 1 (switchable)
            # if len(self.rom_data) <= 0x4000:
            #     data = self.rom_data[addr - 0x4000]
            #     print(f"ROM Bank 1: Address: {addr:04X}, Data: {data:02X}")
            # else:
            rom_offset = self.current_bank * 0x4000
            data = self.rom_data[rom_offset + (addr - 0x4000)]
            # print(
            #     f"ROM Bank: {self.current_bank}, ROM Offset: {rom_offset:04X}, Data: {data:02X}")
            return data
    ...
    def read_word(self, addr):
        ...
        # hi = self.read_byte(addr + 1) if (addr + 1) < len(self.rom_data) else 0
        # return lo | (hi << 8)
